...
type Props = RootStackScreenProps<'TestQuiz'>;

export default function TestQuizScreen({navigation, route}: Props) {
  const theme = useTheme();
  
  // Get the selected quiz from route params
  ...
  const handleQuizSubmit = useCallback(async () => {
    ...
    try {
      ...
      // Calculate simple score for test mode
      let correctCount = 0;
      const totalQuestions = questions.length;
      
      // For test mode, we'll create a simple scoring system
      Object.entries(finalAnswers).forEach(([questionId, answer]) => {
        // Simple scoring - this is just for testing UI
        // In a real app, you'd have correct answers to compare against
        if (Math.random() > 0.3) { // Simulate ~70% correct rate
          correctCount++;
        }
      });
      ...
      if (isMountedRef.current) {
        ...
        const mockQuizResult = {
          ...
          scoreBreakdown: {
            ...
            speed: Math.floor(Math.random() * 20) + 10, // Mock speed points
            total: finalScore
          },
          finishTimeSeconds: (selectedQuiz.estimatedTime * 60) - timeRemaining,
          accuracyPoints: correctCount * 10,
          questions: questions.map((q, index) => ({
            questionId: q.id,
            isCorrect: Math.random() > 0.3, // Mock results
            timeSpent: Math.floor(Math.random() * 30) + 5, // Mock time
          ...
          <View style={styles.heroSection}>
            <View style={[styles.gameLogoContainer, {backgroundColor: 'transparent'}]}>
              <Text style={[styles.gameTitle, {color: theme.colors.primary}]}>
                {selectedQuiz.title.toUpperCase()}
              </Text>
